...
export function InternalMessagingModal({ open, onOpenChange }: InternalMessagingModalProps) {
  ...
  const handleStartConversation = (profile: Profile) => {
    const existingConv = conversations.find(
      c => !c.is_group && c.members?.some(m => m.id === profile.id)
    );
  ...
  const getConversationName = (conv: Conversation) => {
    ...
    if (conv.members && conv.members.length > 0) {
      return conv.members[0].display_name;
    }
    return "Unknown";
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[600px] p-0 flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left sidebar - Conversations/Team */}
                          ...
                          <div className="relative">
                            <Avatar className="h-10 w-10">
                              ...
                              <AvatarFallback className="bg-primary/10 text-primary text-sm">
                                {conv.is_group ? conv.name?.[0] : getInitials(getConversationName(conv))}
                              </AvatarFallback>
                            </Avatar>
                            {!conv.is_group && conv.members?.[0]?.is_online && (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {profile.is_online ? "Online" : `Last seen ${formatTime(profile.last_seen)}`}
                            </p>
                          </div>
                        </div>
                      </button>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {selectedConversation.is_group 
                          ...
                          : selectedConversation.members?.[0]?.is_online ? "Online" : "Offline"
                        }
                      </p>
                    </div>
                  </div>
