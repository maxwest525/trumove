...
export default function InventoryTable({ items, onUpdateItem, onRemoveItem, onClear }: InventoryTableProps) {
  ...
  const handleDownload = () => {
    const content = items.map(item => {
      ...
      return `${item.name},${item.room},${item.quantity},${item.weightEach},${item.quantity * item.weightEach},${cubicFt},${item.quantity * cubicFt}`;
    }).join('\n');
    const header = 'Item,Room,Qty,Weight Each (lbs),Total Weight (lbs),Cu Ft Each,Total Cu Ft\n';
