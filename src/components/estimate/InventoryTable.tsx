...
export default function InventoryTable({ items, onUpdateItem, onRemoveItem, onClear }: InventoryTableProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    // Header with logo text and date
    doc.setFontSize(24);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(34, 197, 94); // Green color
    doc.text("TruMove", 14, 20);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100, 100, 100);
    doc.text(`Generated: ${format(new Date(), "MMMM d, yyyy 'at' h:mm a")}`, pageWidth - 14, 20, { align: "right" });
    
    // Title
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("Your Move Inventory", 14, 35);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100, 100, 100);
    doc.text(`${items.length} items added`, 14, 42);
    ...
    const tableData = items.map(item => {
      ...
      return [
        ...
        `${item.weightEach} lbs`,
        `${cubicFt} cu ft`,
        `${item.quantity * item.weightEach} lbs`,
        `${item.quantity * cubicFt} cu ft`
      ];
    });
    ...
    autoTable(doc, {
      startY: 50,
      head: [['Item', 'Room', 'Qty', 'Weight', 'Cu Ft', 'Total Weight', 'Total Cu Ft']],
      body: tableData,
      foot: [[
        '', '', 'Totals:', '', '', 
    ...
    doc.text("Generated by TruMove - AI-powered moving quotes", 14, finalY + 15);
