...
export default function OnlineEstimate() {
  ...
  const [isAtInventory, setIsAtInventory] = useState(false);
  const inventoryRef = useRef<HTMLDivElement>(null);
  
  const totalCubicFeet = useMemo(() => calculateTotalCubicFeet(items), [items]);

  // Intersection Observer to detect when inventory section is in view
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsAtInventory(entry.isIntersecting);
      },
      { threshold: 0.3 }
    );
  ...
  return (
    <SiteShell>
      <div className="max-w-[1400px] mx-auto px-4 py-8">
        ...
        {!wizardComplete ? (
          // LOCKED STATE: Three-Column Layout - Form | Inventory | Summary Pill (far right)
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr_auto] gap-4 lg:gap-6 items-start">
            {/* Left Column - Wizard (wider) */}
            ...
            {/* Middle Column - Inventory Builder (locked, more space) */}
            ...
            {/* Right Column - Floating Summary Pill (far right) */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
            ...
            <div className="space-y-6">
              {/* Inventory Builder with simple header */}
              <div ref={inventoryRef} className="rounded-2xl border border-border/60 bg-card shadow-lg overflow-hidden">
                {/* Header - Same style as other form sections */}
                <div className="tru-summary-header-compact border-b border-border/40">
            ...
            {/* Right Column - Summary + Inventory List + Finalize */}
            <div className="space-y-4 lg:sticky lg:top-6">
              {/* Move Summary */}
              <QuoteSnapshot items={items} moveDetails={moveDetails} extendedDetails={extendedDetails} />

              {/* Your Move Inventory List with thumbnails */}
              {items.length > 0 && (
