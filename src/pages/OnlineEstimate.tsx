          ...
          <div className="space-y-6">
            {!wizardComplete ? (
              ...
              <div className="tru-estimate-overlay">
                ...
                {/* Semi-visible Preview of Inventory Builder */}
                <div className="tru-estimate-bg-preview mt-6">
                  <section className="rounded-2xl border border-border/60 bg-card p-5 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full">Locked</span>
                    </div>
                    {/* Show actual room tabs preview */}
                    <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                      {['Living Room', 'Bedroom', 'Kitchen', 'Dining Room', 'Office', 'Garage'].map((room) => (
                        <div key={room} className="px-3 py-1.5 rounded-lg bg-muted text-muted-foreground text-xs font-semibold whitespace-nowrap">
                          {room}
                        </div>
                      ))}
                    </div>
                    {/* Show item grid preview */}
                    <div className="grid grid-cols-3 gap-2">
                      {['Sofa', 'Armchair', 'Coffee Table', 'TV Stand', 'Bookcase', 'Lamp'].map((item) => (
                        <div key={item} className="p-3 rounded-lg bg-muted/50 border border-border/40 text-center">
                          <div className="text-xs font-medium text-muted-foreground">{item}</div>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 text-center text-sm text-muted-foreground">
                      Complete the form above to start building your inventory
                    </div>
                  </section>
                </div>
              </div>
