...
import { Lock, MapPin, Calendar, Home, Route, Package, Scale } from "lucide-react";
...
Generated by TruMove Online Estimate Tool
  ...
  // Calculate totals for inline summary
  const totalWeight = calculateTotalWeight(items);
  const effectiveMoveType = moveDetails.moveType === 'auto' 
    ? (moveDetails.distance >= 150 ? 'long-distance' : 'local')
    : moveDetails.moveType;
    ...
    <SiteShell>
      <div className="max-w-[1400px] mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left Column - Wizard + Inline Summary */}
          <div className="space-y-4">
            {/* Wizard Card */}
            <div className="tru-floating-form-card tru-floating-form-compact lg:max-w-none">
              <EstimateWizard onComplete={handleWizardComplete} />
            </div>

            {/* Inline Move Summary */}
            <div className="tru-inline-summary">
              <div className="tru-inline-summary-header">
                <Route className="w-4 h-4 text-primary" />
                <span>Move Summary</span>
              </div>
              <div className="tru-inline-summary-grid">
                <div className="tru-inline-summary-item">
                  <MapPin className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">From</span>
                  <span className="tru-inline-summary-value">{moveDetails.fromLocation || "—"}</span>
                </div>
                <div className="tru-inline-summary-item">
                  <MapPin className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">To</span>
                  <span className="tru-inline-summary-value">{moveDetails.toLocation || "—"}</span>
                </div>
                <div className="tru-inline-summary-item">
                  <Route className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">Distance</span>
                  <span className="tru-inline-summary-value">{moveDetails.distance > 0 ? `${moveDetails.distance.toLocaleString()} mi` : "—"}</span>
                </div>
                <div className="tru-inline-summary-item">
                  <Calendar className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">Date</span>
                  <span className="tru-inline-summary-value">{moveDetails.moveDate || "—"}</span>
                </div>
                <div className="tru-inline-summary-item">
                  <Package className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">Items</span>
                  <span className="tru-inline-summary-value">{items.length > 0 ? `${items.length} items` : "—"}</span>
                </div>
                <div className="tru-inline-summary-item">
                  <Scale className="w-3.5 h-3.5" />
                  <span className="tru-inline-summary-label">Weight</span>
                  <span className="tru-inline-summary-value">{totalWeight > 0 ? `${totalWeight.toLocaleString()} lbs` : "—"}</span>
                </div>
              </div>
              <div className="tru-inline-summary-estimate">
                <span className="tru-inline-summary-estimate-label">Estimated Cost</span>
                <span className="tru-inline-summary-estimate-value">TBD</span>
              </div>
            </div>
